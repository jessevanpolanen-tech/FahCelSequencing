...
export function fromLine() {
  const email = process.env.FROM_EMAIL || 'sales@fahcel.eu';
...
export async function sendEmail({ to, subject, text, html, tags = [], replyTo }) {
  ...
  const body = {
    ...
    subject,
    text,
    reply_to: replyTo || process.env.REPLY_TO || 'sales@fahcel.eu',
    tags,
  };
...
// NOTIFY_TO (falls back to FORWARD_TO, then sales@fahcel.eu). Reply-To is set to
...
export async function notifyNewLead(lead = {}, source = 'dashboard') {
  const to = process.env.NOTIFY_TO || process.env.FORWARD_TO || 'sales@fahcel.eu';
