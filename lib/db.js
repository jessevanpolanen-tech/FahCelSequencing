...
// `nextDueAt` is null when the sequence completes. next_due_at is NOT NULL in
// the schema, so writing that null threw and the whole advance rolled back —
// leaving the enrollment 'active' on its final step with a due time in the
// past, which re-sent the last email on every tick, forever. Nothing ever
// reached 'completed'. Keep the existing timestamp instead; dueEnrollments()
// filters on status = 'active', so a completed row is excluded regardless.
export async function advanceEnrollment(id, { stepIndex, nextDueAt, status }) {
  await sql`
    update enrollments set
      step_index = ${stepIndex},
      next_due_at = coalesce(${nextDueAt ? nextDueAt.toISOString() : null}::timestamptz, next_due_at),
      status = ${status},
      updated_at = now()
    where id = ${id};`;
}
